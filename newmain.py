...
if "faq_added" not in st.session_state:
    st.session_state.faq_added = False

if "faq_message" not in st.session_state:
    st.session_state.faq_message = ""
...
if System_Maintenance == "Y":
    ...
    with tab1:
        ...
        if prompt := chosen_prompt:
            ...
            # Button to add user query to FAQs
            if st.button("Add to FAQs"):
                if chosen_prompt:
                    if chosen_prompt not in questions["Question"].values:
                                new_query = pd.DataFrame({"Question": [chosen_prompt]})
                                questions = pd.concat([questions, new_query], ignore_index=True)
                                questions.to_csv(select_database_table_question_csv, index=False)                                
                                st.success("Your query has been added to the FAQs!")
                                st.session_state.faq_added = True
                                st.session_state.faq_message = "Your query has been added to the FAQs!"

                    else:
                        st.session_state.faq_added = True
                        st.session_state.faq_message = "This query already exists in the FAQs."
                else:
                    st.session_state.faq_added = True
                    st.session_state.faq_message = "Please enter a query before adding to FAQs."
        elif prompt := text:
            #st.session_state.user_prompt = prompt
            st.session_state.messages.append({"role": "user", "content": prompt})
            with st.chat_message("user"):
        ...
        if "response" in st.session_state and "tables_data" in st.session_state:
                ...
                for table, data in st.session_state.tables_data.items():
                        ...
                        if not data.empty:
                            ...
                            # Auto-select x-axis and y-axis
                            x_axis = datetime_cols[0] if datetime_cols else (categorical_cols[0] if categorical_cols else numerical_cols[0])
                            y_axis = numerical_cols[0] if numerical_cols else None  # Only select numerical for Y

                            # Auto-select appropriate chart type
                            if y_axis:
                                if x_axis in categorical_cols or x_axis in datetime_cols:
                                    chart_type = "Bar Chart"  # Categorical/Datetime vs Numerical → Bar Chart
                                elif x_axis in numerical_cols and y_axis in numerical_cols:
                                    chart_type = "Scatter Plot"  # Numerical vs Numerical → Scatter Plot
                                else:
                                    chart_type = "Line Chart"  # Default to Line Chart
                            else:
                                chart_type = "Histogram"  # If only one numerical column

                            # Display selections (users can override if needed)
                            x_axis = st.selectbox(f"## **Select X-axis for {table}**", data.columns, index=data.columns.get_loc(x_axis), key=f"x_axis_{table}")
                            y_axis = st.selectbox(f"**Select Y-axis for {table}**", data.columns, index=data.columns.get_loc(y_axis) if y_axis else 0, key=f"y_axis_{table}")
                            chart_type = st.selectbox(f"**Select Chart Type for {table}**", 
                                                    ["Line Chart", "Bar Chart", "Scatter Plot", "Pie Chart", "Histogram", 
                                                    "Box Plot", "Heatmap", "Violin Plot", "Area Chart", "Funnel Chart"], 
                                                    index=["Line Chart", "Bar Chart", "Scatter Plot", "Pie Chart", "Histogram", 
                                                            "Box Plot", "Heatmap", "Violin Plot", "Area Chart", "Funnel Chart"].index(chart_type),
