...
#synapse_db_schema = os.getenv("synapse_db_schema")
...
def get_chain(question, _messages, selected_model):
    ...
    print("chainnnnnnnn",itemgetter("question"))
    ...
    # if configure.selected_subject.startswith('Adv'):
            ...
            print("DB Connection Done for Adventureworks----",db._schema)
    elif st.session_state.selected_subject.startswith('Synapse'):
            db = SQLDatabase.from_uri(f'mssql+pyodbc:///?odbc_connect={quote_plus(connection_string)}',
                        #    include_tables=chosen_tables,
                           view_support=True,
                           sample_rows_in_table_info=1,
                           lazy_table_reflection=True)
...
def invoke_chain(question, messages, selected_model):
    try:
            ...
            elif configure.selected_subject.startswith('Synapse'):
                alchemyEngine = create_engine(f'mssql+pyodbc:///?odbc_connect={quote_plus(connection_string)}')
            # elif configure.selected_subject.startswith('Dummy'):
        ...
        # }
            ...
            for table in chosen_tables:
                ...
                # df = pd.read_sql(sql=query, con=db)  # Use db directly
